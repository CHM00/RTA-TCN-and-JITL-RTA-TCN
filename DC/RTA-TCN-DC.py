...
TRAIN_SIZE = 1648 + SEQ_LEN - 1
# TRAIN_SIZE = 961 + SEQ_LEN - 1
#TRAIN_SIZE = 1000
...
mdl = TCANModel(input_size=7, output_size=1, num_channels=[40, 40, 40], seq_length=SEQ_LEN, n_epoch=240, batch_size=64,
                lr=0.001, seed=1024, device=device).fit(train_X, y_train)  # input_size=8 是加y               # epoch=240
...
#np.savetxt('TA-TCN y_pred.txt', y_pred)
np.savetxt('OFFLINE y_pred.txt', y_pred)
...
np.savetxt('OFFLINE y_true.txt', y_test)
print("总耗时: ", end - start)
