...
model = TCANModel(input_size=5, output_size=1, num_channels=[25, 25, 25], seq_length=SEQ_LEN, n_epoch=100, batch_size=64, lr=0.001,seed=1024, device=device)
for n in range(0, len(test_X[:, 0])):
    ...
    for m in range(0, window):
        ...
        if index < 24:
            input.append(history_x[0:25, :])
            output.append(history_y[24, :].reshape(-1, 1))
